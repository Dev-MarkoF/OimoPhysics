...
class GenericJoint extends Joint {
	...
	public function new(config:GenericJointConfig) {
		...
		if (config.localBasis1.determinant() < 0 || config.localBasis2.determinant() < 0)
			trace("[warning] joint basis must be right handed");
		var lb1:IMat3;
		var lb2:IMat3;
	...
	public inline function getRotationalSpringDampers():Array<SpringDamper> {
		return _rotSds.toArray(); // fehm - issue of Io-th in github, wrong return value changed to _rotSds instead of _translSds
	}
